      ...
      <div className="mx-auto max-w-3xl px-5 py-10">
        ...
        <div className="mb-12 flex items-center justify-between">
          <div className="inline-flex items-baseline text-lg font-black tracking-tight">
            <span className="text-white">PleaseBeReady</span>
            <span className="text-emerald-400">.com</span>
          </div>
          <a href="#start" className="rounded-full bg-emerald-500 px-4 py-2 text-xs font-black uppercase tracking-[0.15em] text-emerald-950 hover:bg-emerald-400 transition">
            Start Here
          </a>
        </div>
        ...
        <section className="mb-14 text-center">
          ...
          <p className="mx-auto mb-4 max-w-xl text-lg font-semibold leading-8 text-emerald-100">
            A calm, friendly guide to getting your household ready — one small step at a time.
          </p>
          <p className="mx-auto max-w-lg text-sm font-semibold leading-7 text-emerald-300/80">
        ...
        {/* Ethos */}
        <section className="mb-14 rounded-3xl border border-emerald-400/15 bg-emerald-900/30 p-7 text-center">
          <p className="text-base font-semibold leading-8 text-emerald-100">
        ...
        <section id="start" className="mb-16 scroll-mt-6">
          <h2 className="mb-2 text-2xl font-black text-white">Your First Weekend</h2>
          <p className="mb-6 text-sm font-semibold text-emerald-300/80">Five small wins. Do them this weekend and you&apos;re already ahead of most people.</p>
          <div className="flex flex-col gap-3">
            {startHere.map((s) => (
              <div key={s.n} className="flex gap-4 rounded-2xl border border-emerald-400/15 bg-emerald-900/20 p-5">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-base font-black text-emerald-950">{s.n}</span>
                <div>
                  <h3 className="text-base font-black text-white">{s.title}</h3>
                  <p className="mt-1 text-sm font-semibold leading-6 text-emerald-200/80">{s.text}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-16">
          <h2 className="mb-2 text-2xl font-black text-white">The 10 Readiness Areas</h2>
          <p className="mb-6 text-sm font-semibold text-emerald-300/80">You don&apos;t have to tackle these all at once. Pick one, check it off, come back next month.</p>
          <div className="grid gap-4 sm:grid-cols-2">
            {areas.map((a) => (
              <div key={a.title} className="rounded-2xl border border-emerald-400/15 bg-emerald-900/20 p-6">
                <div className="mb-3 text-3xl">{a.emoji}</div>
                <h3 className="text-lg font-black text-white">{a.title}</h3>
                <p className="mt-1 mb-4 text-sm font-semibold leading-6 text-emerald-200/70">{a.intro}</p>
                <ul className="flex flex-col gap-2">
                  {a.items.map((it) => (
                    <li key={it} className="flex gap-2 text-sm font-semibold text-emerald-100">
        ...
        {/* Levels */}
        <section className="mb-16">
          <h2 className="mb-2 text-2xl font-black text-white">Pick Your Level</h2>
          <p className="mb-6 text-sm font-semibold text-emerald-300/80">Start where you are. Move up only when you&apos;re ready. Every level is a real win.</p>
          <div className="grid gap-4 sm:grid-cols-3">
            {levels.map((l) => (
              <div key={l.title} className="rounded-2xl border border-emerald-400/15 bg-emerald-900/20 p-6">
                <p className="text-xs font-black uppercase tracking-[0.15em] text-emerald-400">{l.tag}</p>
                <h3 className="mt-1 text-xl font-black text-white">{l.title}</h3>
                <p className="mt-2 text-sm font-semibold leading-6 text-emerald-200/70">{l.text}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Resources */}
        <section className="mb-14 rounded-3xl border border-emerald-400/15 bg-emerald-900/20 p-7">
          <h2 className="mb-2 text-xl font-black text-white">Trusted, Free Resources</h2>
          <p className="mb-5 text-sm font-semibold text-emerald-300/80">When you want to go deeper, these are the real, official sources.</p>
          <div className="flex flex-col gap-2">
            {resources.map((r) => (
              <a key={r.href} href={r.href} target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-between rounded-xl border border-emerald-400/10 bg-emerald-950/40 px-5 py-3.5 text-sm font-black text-emerald-100 hover:border-emerald-400/30 hover:text-white transition">
                <span>{r.label}</span>
        ...
        <section className="mb-12 text-center">
          <p className="mx-auto max-w-lg text-lg font-black leading-8 text-white">
            Being ready is a gift you give your family — and your neighbors.
          </p>
          <p className="mx-auto mt-3 max-w-md text-sm font-semibold text-emerald-300/80">
            Start with one thing today. That&apos;s all it takes to begin.
          </p>
        </section>

        {/* Disclaimer + footer */}
        <footer className="border-t border-emerald-400/10 pt-8 text-center">
          <p className="mx-auto mb-5 max-w-xl text-xs font-semibold leading-6 text-emerald-400/50">
          ...
          <p className="text-sm font-black tracking-tight">
            <span className="text-emerald-200">PleaseBeReady</span>
            <span className="text-emerald-400">.com</span>
          </p>
        </footer>
